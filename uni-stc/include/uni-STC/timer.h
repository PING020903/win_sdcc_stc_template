...
// depending on the number of pins and MCU family.
	...
	#define TIMER_HAS_BRT
		...
		// The rule is that MCU with 2 timers have T0 and T2, EXCEPT
...
SFR(T0H, 0x8C);
// NOTE: T0L must be accessed first when writing a 16-bit value (or just use T0).
...
#define P_T0PR 1
	...
	SFR(T1H, 0x8D);
	// NOTE: T1L must be accessed first when writing a 16-bit value (or just use T1).
	...
	#define P_T1PR 3
		...
		SFR(T2H, 0xCD);
		// NOTE: T2L must be accessed first when writing a 16-bit value (or just use T2).
...
#define P_EN_WDT 5
	...
	// SFR T4L: Timer 4 low
	SFR(T4L, 0xD3);
	// NOTE: T4L must be accessed first when writing a 16-bit value (or just use T4).
	...
	// SFR T3L: Timer 3 low
	SFR(T3L, 0xD5);
	// NOTE: T3L must be accessed first when writing a 16-bit value (or just use T3).
...
#ifdef TIMER_HAS_T11
	// SFR T11CR: T11 control register
	SFRX(T11CR, 0xFE78);
	...
	#define M_T11CLKS 0x0c
	#define P_T11CLKS 2
	
	#define M_T11x12 0x10
	#define P_T11x12 4
	
	#define M_T11CLKO 0x20
	#define P_T11CLKO 5
	
	#define M_T11_C_T 0x40
	#define P_T11_C_T 6
	
	#define M_T11RUN 0x80
	#define P_T11RUN 7
	
	typedef enum {
		T11Clock_SysClk = 0,
		T11Clock_InternalHighSpeedRC,
		T11Clock_External32KCrystal,
		T11Clock_InternalLowSpeedRC,
	} T11ClockSource;
	
	// SFR TM11PS: Timer 11 clock prescaler register
	SFRX(TM11PS, 0xFE79);
	
	// SFR T11H: Timer 11 high
	SFRX(T11H, 0xFE7A);
	// SFR T11L: Timer 11 low
	SFRX(T11L, 0xFE7B);
	// NOTE: T11L must be accessed first when writing a 16-bit value.
#endif
